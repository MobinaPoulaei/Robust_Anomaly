...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        if isinstance(self.attn, Attention):
            x = x.transpose(0, 1)
            [x, x_ori], attn_weights = self.attn(x) #attnweights: [1, head, 1370, 1370],
            # x: [1, 1370, d] localized, xori: global, 1370: patches
    ...
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, need_weights: bool = False,
        ...
        if self.text_layer and (design_details is not None):
            self.resblocks = nn.ModuleList(
                [ResidualAttentionBlock_learnable_token(width, heads, attn_mask, design_details, text_layer, i=i) for i
                 in range(layers)])
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch=False, proj_use=True, DPAM_layer=None, ffn=False):
        ...
        if True:
            ...
            self.activations = patch_tokens[-1]
