...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        out_test = model.encode_image(image, features_list, DPAM_layer=20)
        attn_weights = model.visual.transformer.resblocks[-1].attn_weights
        avg_attn_weights = attn_weights.mean(dim=1)  # Shape: [batch_size, num_tokens, num_tokens]

        cls_attn_map = avg_attn_weights[0, 0, 1:]  # Shape: [num_tokens - 1]

        num_patches = int(cls_attn_map.size(0) ** 0.5)  # Assuming square grid of patches
        cls_attn_map = cls_attn_map.reshape(num_patches, num_patches)
        plt.imshow(cls_attn_map.cpu().numpy(), cmap='viridis')
        plt.colorbar()
